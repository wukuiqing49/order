...
public class PermissionChecker {
    ...
    public static boolean checkPermissions(final Activity activity, int leftButton, int rightButton, String[] permissions
            , final int requestCode, final int dialogMsgForRationale, final boolean activityFinish) {
        //Android6.0以下默认有权限
        ...
        if (needList.size() != 0) {
            if (needShowRationale) {
                AlertDialogUtils.showTwoButtonDialog(
                        activity,
                        activity.getString(leftButton),
                        activity.getString(rightButton),
                        activity.getString(dialogMsgForRationale),
    ...
    public static boolean[] onRequestPermissionsResult(final Activity activity, int requestCode, String[] permissions, int[] grantResults,
                                                       final boolean cencelFinish, final boolean confirmFinish, int dialogMsgForNerverAsk) {
        boolean result = true;
...
//            Toast.makeText(activity, R.string.toast_imagepicker_permission_denied, Toast.LENGTH_SHORT).show();
            if (isNerverAsk) {
                AlertDialogUtils.showTwoButtonDialog(
                        activity,
                        activity.getString(R.string.dialog_imagepicker_permission_nerver_ask_cancel),
                        activity.getString(R.string.dialog_imagepicker_permission_nerver_ask_confirm),
                        activity.getString(dialogMsgForNerverAsk),
                        R.color.color_dialog_btn, R.color.color_ffa300, new AlertDialogUtils.DialogTwoListener() {
                            @Override
                            public void onClickLeft(Dialog dialog) {
                                dialog.dismiss();
