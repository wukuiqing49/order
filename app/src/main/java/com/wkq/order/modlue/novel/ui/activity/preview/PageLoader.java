...
public class PageLoader {
    ...
    private void initData() {
        //初始化参数
        mMarginWidth = ScreenUtils.dp2px(mContext, DEFAULT_MARGIN_WIDTH);
        mMarginHeight = ScreenUtils.dp2px(mContext, DEFAULT_MARGIN_HEIGHT);
        mIntervalSize = ScreenUtils.dp2px(mContext, DEFAULT_INTERVAL);
        mParagraphSize = ScreenUtils.dp2px(mContext, DEFAULT_PARAGRAPH_INTERVAL);
        contentMarginTop = ScreenUtils.dp2px(mContext, DEFAULT_CONTENT_MARGIN_TOP);
        contentMarginBottom = ScreenUtils.dp2px(mContext, DEFAULT_CONTENT_MARGIN_BOTTOM);
    }
...
//        mTipPaint.setColor(mPageView.getTextColor());
        ...
        mTipPaint.setTextSize(ScreenUtils.sp2px(mContext, DEFAULT_TIP_SIZE));//Tip默认的字体大小
...
//            mPageView.drawCurPage(true);
   ...
   /*     mBookRecord.setBookId(mCollBook.get_id());
    ...
    private int tipMarginHeight = ScreenUtils.dp2px(OrderApplication.getContext(), 3);
    ...
    public void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            //擦除区域
            mBgPaint.setColor(mPageView.getPageBackground());
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + ScreenUtils.dp2px(mContext, 2), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
    ...
    private void drawTime(Canvas canvas, boolean needClean) {
        ...
        float tipTop = tipMarginHeight - mTipPaint.getFontMetrics().top;
        int outFrameLeft =
                mDisplayWidth - mMarginWidth - ScreenUtils.dp2px(mContext, 2) - (int) mTipPaint.measureText("xxx");
        if (hasFixedHair) {
...
//        float y = mDisplayHeight - mTipPaint.getFontMetrics().bottom - tipMarginHeight;
        ...
        float x = outFrameLeft - mTipPaint.measureText(time) - ScreenUtils.dp2px(mContext, 4);
    ...
    private void drawBattery(Canvas canvas, boolean needClean) {
        ...
        int polarHeight = ScreenUtils.dp2px(mContext, 6);
        int polarWidth = ScreenUtils.dp2px(mContext, 2);

        float tipTop = tipMarginHeight + mTipPaint.getFontMetrics().bottom + ScreenUtils.dp2px(mContext, 1);
...
//            }
    ...
    public boolean prevChapter() {
        ...
        if (!mAdapter.hasPreviousSection(mCurChapterPos)) {
            AlertUtil.showDeftToast(mContext, "已经没有上一章了");
            return false;
        }
    ...
    public boolean nextChapter() {
        ...
        if (!mAdapter.hasNextSection(mCurChapterPos)) {
            AlertUtil.showDeftToast(mContext, "已经没有下一章了");
            return false;
        }
...
//        int nextChapter = mCurChapterPos + 1;
    ...
    private boolean checkStatus() {
        if (mStatus == STATUS_LOADING) {
            AlertUtil.showDeftToast(mContext, "正在加载中，请稍等");
            return false;
